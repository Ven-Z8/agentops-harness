...
def test_board_export_writes_both_snapshots_only_after_fake_remote_validation(
    ...
    from app.project_control import cli
    from tests.unit.test_project_control_github import PROJECT_RESPONSE, FakeTransport
    ...
    (tmp_path / "coordination/CURRENT.md").write_text("last current\n", encoding="utf-8")
    (tmp_path / "coordination/BOARD.md").write_text("last board\n", encoding="utf-8")
    ...
    assert "AO-D01-01" in (tmp_path / "coordination/BOARD.md").read_text(encoding="utf-8")
    assert "AO-D01-01" in (tmp_path / "coordination/CURRENT.md").read_text(encoding="utf-8")
    assert transport.calls


def test_board_export_refuses_null_config_and_preserves_both_snapshots(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    from app.project_control import cli
...
def test_board_export_refuses_cross_repository_items_before_writes(
    ...
    current.write_text("last current\n", encoding="utf-8")
    board.write_text("last board\n", encoding="utf-8")
    response = yaml.safe_load(yaml.safe_dump(PROJECT_RESPONSE))
    response["data"]["user"]["projectV2"]["items"]["nodes"][0]["content"]["repository"][
        "nameWithOwner"
    ] = "Ven-Z8/other-repository"
